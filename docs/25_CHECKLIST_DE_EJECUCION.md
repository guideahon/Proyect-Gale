...
Estado actual verificado: import limpio, 438/438 assertions en 19 archivos, 16 schemas + 6 ejemplos + 1 template + 33 casos, índice al día.
...
- [x] **T0.4 — `project.godot` real.** Autoloads `Bootstrap` y `UpdateService`, `renderer/rendering_method="mobile"`, icono, versión semver, y **OpenXR habilitado sólo en Android** (`openxr/enabled.android=true`): con el valor global en true cada corrida headless intenta abrir el runtime XR de la máquina, falla con `XR_ERROR_FORM_FACTOR_UNAVAILABLE` y la batería pasa de 2 s a más de 500. **Test:** `tests/unit/test_project_settings.gd` valida los tres valores admitidos de `rendering_method`, la sección `[xr]`, autoloads, escena principal y versión; verificado en rojo poniendo `"vulkan_mobile"`. Los comentarios van con `;`: `ConfigFile` no entiende `#` y se come la clave siguiente. **Pendiente sólo la decisión de ADR-011**, que sale de medir S2 en el visor.
...
- [x] **T0.8 — Workflow `pr.yml`.** Corre en push a cualquier rama y en PR a main; antes sólo disparaba en PR a `main` y **nunca se había ejecutado**. **Test:** run verde en GitHub Actions (commit `bb98b49`): lockfile, import, tests, schemas, índice y recursos, todos success. Cuatro defectos salieron a la luz en las primeras corridas: `$env:TEMP` inexistente en Linux, la GDExtension del addon abortando Godot al salir (exit 134) tras un import exitoso, el índice escaneando disco en vez de `git ls-files`, y `python` no garantizado en el runner.
- [~] **T0.9 — Workflow `release.yml`.** Reescrito con flags reales: templates verificados por SHA-256, build template de Gradle, addon fijado, keystore desde secreto y export con `--export-release "Quest" <path>`. La versión anterior no podía funcionar (`--export-template "Android"`, `--export-id 0`, binario borrado antes de usarse, sin firma, y `$(...)` sin evaluar en el cuerpo del release). Variables de entorno del keystore verificadas contra los strings del binario. **No verificado:** probarlo exige publicar un release, que es una acción hacia afuera; se agregó `workflow_dispatch` para que un humano lo dispare. **Bloqueado además por** el secreto `ANDROID_KEYSTORE_BASE64`, que sólo puede cargar Cristian.
...
- [x] **T3.10 — `override_engine.gd`.** `extend`, `patch`, `replace` con advertencia, `disable` con placeholder, **según la semántica de `docs/09`**: EXTEND suma sin destruir (claves nuevas, listas concatenadas, escalar existente intacto) y PATCH pisa las propiedades indicadas. La primera versión los tenía invertidos —EXTEND sobrescribía, que es justo lo que la spec prohíbe— y guardaba un solo override por target, así que dos mods que extendían la misma definición se pisaban. **Test:** 24/24, incluye composición de dos EXTEND en orden de carga y conflicto sólo cuando interviene REPLACE o DISABLE.
- [x] **T3.11 — `mod_error.gd`.** Errores estructurados con mod causante y motivo. **Test:** 3/3 — report/get/clear.
- [x] **T3.12 — Modo seguro.** Contador de fallos consecutivos, arranque sólo con contenido oficial, sin borrado automático. `should_load_mod` exige que el paquete oficial venga con **firma verificada contra la clave anclada** (`docs/24` §7): el nombre no prueba procedencia, cualquiera puede llamar a su mod `official.loquesea`. **Test:** `tests/unit/test_safe_mode.gd` 14/14 — tres fallos activan, un éxito resetea, un impostor con nombre oficial y sin firma se rechaza, y un archivo de mod en `user://mods` sobrevive intacto a la activación.
- [~] **T3.13 — `official.base` registra la isla de M2.** `official_mods/official.base/` con `register.gd` y `manifest.json` que valida contra el schema. **Estaba en `core/mods/official_base.gd`**, o sea contenido dentro del núcleo: lo prohíbe la regla de oro de `docs/00` y `docs/08` §92 («no se permite hardcodear directamente una espada, misión o isla dentro del núcleo»). Movido a `official_mods/`, que es donde el contenido oficial usa las mismas APIs públicas que un mod comunitario (ADR-004). **Test:** registrar y consultar la isla, 4 assertions en `tests/integration/test_multi_pck.gd`. **Pendiente:** que se cargue de verdad desde un PCK montado, que depende de T3.14.
- [~] **T3.14 — Tests de integración.** `tests/integration/test_multi_pck.gd`, 13 assertions: orden de carga con tres mods, overrides encadenados con dos REPLACE y su conflicto, modo seguro ante mods defectuosos, y registro de `official.base`. El runner ya descubre `tests/integration/` además de `tests/unit/`. **Pendiente el montaje real de PCKs:** los fixtures `.gmod` llevan un `content.pck` de relleno, no un PCK generado por Godot, así que `pack_mounter` los rechaza; hace falta generar fixtures con PCKs reales.
- [~] **T3.15 — Validador CLI headless.** `tools/ci/validate_schemas.gd` reusa `schema_validator.gd` y da **el mismo veredicto** que el script de Python: `OK: 16 schemas, 6 ejemplos, 1 templates, 33 casos` en los dos. La primera versión no funcionaba: llamaba a `validate(schema, doc)` cuando la firma es `validate_by_name(nombre, doc)`, recorría `examples/` sin entrar en subdirectorios (validaba cero archivos en silencio), no mapeaba `manifest.json`, y al abortar por el error de API nunca llegaba al `quit()`, así que colgaba el proceso; además tenía un `quit(1)` sin `return` que dejaba pasar el fallo con código 0. **No se retira `check_schemas.py` todavía:** valida los schemas contra el metaschema oficial de Draft 2020-12, algo que el validador GDScript no hace. Retirarlo perdería esa comprobación.
